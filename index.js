...
/* =====================================================
   ROOT (Imebadilishwa ioneshe index.ejs)
===================================================== */
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      // Safisha namba: ondoa alama ya kujumlisha (+), nafasi, na vistari vyote
      ...
      // Anzisha session kama haijaanza bado
      let sock = getSock(sessionId);
      if (!sock) {
        sock = await startSock(sessionId, "api-pair");
      }

      // Subiri sekunde chache ili socket ikae sawa kabla ya kuomba code
      await new Promise((resolve) => setTimeout(resolve, 3000));

      sock = getSock(sessionId);
      if (!sock) {
        return res.status(500).json({
          success: false,
          error: "Failed to initialize socket for pairing",
        });
      }
      ...
      if (typeof sock.requestPairingCode !== "function") {
        return res.status(500).json({
          success: false,
          error: "requestPairingCode function is not available on this socket version",
        });
      }

      // Omba pairing code kutoka WhatsApp
